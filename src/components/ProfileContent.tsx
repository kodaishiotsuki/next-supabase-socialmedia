...
const ProfileContent = ({
  ...
  //ユーザーIDに紐づいた投稿を取得する関数
  const userPosts = useCallback(
    async (userId: string) => {
      const { data } = await supabase
        .from("posts")
        .select("id,content,created_at,author, profiles(id,name,avatar)")
        .eq("author", userId);
      return data;
    },
    [supabase]
  ...
  //ユーザーIDに紐づいた投稿とプロフィールを取得する関数
  const loadPosts = useCallback(async () => {
    const posts = (await userPosts(userId)) as post[];
    const profile = await userProfile(userId);
    setPosts(posts);
    setProfile(profile);
  }, [userPosts, userProfile, userId]);
